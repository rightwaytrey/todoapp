...
| `GET /api/widget` | Finished rows for the home-screen widget: text, due label, overdue flag, already sorted and capped. Round 6 adds `prefs.widget.group_by` — `"due"` (round 5, unchanged) or `"category"`, which reorders the same rows into category runs (`categories.order` first, then the rest alphabetically, no category last as `""`) and fills `category` on every row whatever `show_category` says — echoed back as a top-level `group_by`. Round 8 adds two more top-level keys for the widget's own category picker: `category` (the active filter, `prefs.widget.category`) and `categories` (the chips to offer — `categories.order` minus anything hidden, then the in-use categories not in that order, alphabetically case-folded; hidden ones are excluded except the active category, which is always offered so its chip can always be un-lit). |
| `POST /api/widget/category` | `{"category": <name> or null}` → `204`. Sets `prefs.widget.category` and nothing else — one intent for the widget's one shot at the network, rather than a `GET` + `PUT` of the whole document. Same validation as that field in `PUT /api/prefs`. |
